...
export function PostCard({body, title, issueNumber, createdAt}: PostCardProps){
    ...
    return (<div onClick={() => navigate(`/github-blog/details/${issueNumber}`)} className="w-[416px] h-[260px] cursor-pointer bg-blue-600 rounded-[10px] p-9 flex flex-col gap-5">
        <div className="flex justify-between">
            <h2 className=" max-w-[283px] font-display font-bold text-xl leading-[160%] text-white-100 ">{title}</h2>
            
            <span className="font-display text-[14px] leading-[160%] text-cyan-100">{createdAt ?formatDistanceToNow(createdAt,{addSuffix: true, locale: ptBR}) :  ''}</span>
        </div>
